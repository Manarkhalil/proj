		...
		<div id="container">
			...
			<div id="content">
				...
				<div id=con1>
					<div id="best">
						<?php
							$q=mysql_query("select * from prod order by sales desc limit 1");	//$q=$con->query("select * from products order by sales desc limit 1");
							if($q)
							{
					...
					<img class="c" src="images/picture12.jpg">
					<img class="c" src="images/picture11.jpg">
					
				</div>
				<?php
					$qc=mysql_query("select * from category");
					if($qc)
					{
						while($rc=mysql_fetch_assoc($qc))
						{
							$cat_id=$rc['cat_id'];
							$cat_name=$rc['name'];
							$qp=mysql_query("select * from prod where cat_id=$cat_id order by prod_id desc limit 1");
							if($qp)
							{
				...
				<br clear=left>
				<?php
					$q=mysql_query("select * from prod order by prod_id desc limit 3");	//latest 3 products
					if($q)
					{
