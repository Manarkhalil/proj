				...
				<div id=cart>
					...
					<?php
						if($_SESSION['cart'])
						{
							...
							echo "$count <b style='color:purple'>item Total: </b>";
							$total=0;		
			...
			<div id="content" >
				...
				if($_POST['h'])
				{
					...
					else
					{
						...
						{
							$date=time();
							$q=mysql_query("insert into order values('',$_SESSION[user_id],
							'$name','$code','$company','$street','$line2','$town','$country','$phone','$date','$total')");
							if($q)
								{
									foreach($_SESSION['cart'] as $k=>$v)
										$q=mysql_query("update prod set sales=sales+$v where prod_id='$k'");
									unset($_SESSION['cart']);
				...
				function show($err='')
				{
					echo "<div style='width:305px;height:300px;float:left;text-align:center;margin-left:30px;background-color:#ad59b9;'>	<h2>Delivery Details</h2>
						...
						<tr><td>Type: </td>";
						global $prod_id;
						$q=mysql_query("select category.name as cat_name,prod.name as prod_name from category,prod where prod.prod_id=$prod_id and category.cat_id=prod.cat_id");
						$r=mysql_fetch_assoc($q);
